...
if ($genomes || $genomesFiles)
{
	...
	if($reference){
		my @tmpl = `ls -d $dbBasePath/NCBI_genomes/$reference*`;
		chomp @tmpl;
...
## Prepare Reference and control file
    system("cp -R $dbBasePath/SNPdb/${SNPdbName}/* $outputDir_abs_path/.") if (! $genomes && $SNPdbName);
...
sub extract_ref_seq {
	...
	foreach my $gbk (@gfiles){
		...
		system("genbank2fasta.pl $gbk >> $out_file");
